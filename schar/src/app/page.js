...
// 将引用面板拆分为独立组件并记忆化
const ReferencePanel = memo(({ showReference, onClose }) => {
  return (
    <div
...
// 移动端右侧四方格按钮
const MobileGridButton = memo(({ onClick }) => {
  return (
    <button
      onClick={onClick}
      className='absolute top-6 right-6 z-10 w-5 h-5 grid grid-cols-2 grid-rows-2 gap-0.5'
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const resizeObserver = new ResizeObserver((entries) => {
      // 使用 requestAnimationFrame 批处理布局计算
